...
summarize_email_prompt_template = ChatPromptTemplate.from_template(
    ...
    Guidelines:
      ...
      2. Generate a list of tags about the email, make sure each tag is simple and each word in tag start with big letter 
...
def update_report_with_gmail_message(user_id: str, account_id: str, account_email: str, report_id: str,
    ...
    if gmail_list:
        ...
        for raw_gmail in gmail_list:
            extracted_gmail = extract_gmail_info(raw_gmail.raw_email, raw_gmail.message_id, raw_gmail.thread_id)
        ...
        with get_session(write=True) as session:
            ...
            for email_record, report_item in zip(email_db_records, mail_report_item_list):
                report_item.id = email_record.id
...
def update_report_with_outlook_emails(user_id: str, account_id: str, account_email: str, report_id: str,
                                      outlook_emails: List[Message], message_num_to_decrease: int):
    if outlook_emails:
        with get_session(write=False) as session:
            # get user focused tags
            user_setting = UserSetting.get_by_user_id(session, user_id)
        ...
        with get_session(write=True) as session:
            ...
            for email_record, mail_report_item in zip(email_db_records, mail_report_item_list):
                mail_report_item.id = email_record.id
